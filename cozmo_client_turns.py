...
def post_status(robot: cozmo.robot.Robot):
    ...
    if trip:
        ...
        column: int = (y + cell_length / 3) // cell_length
    ...
    if trip:
        ...
        if cell['shape'] == 'tRight':
            ...
            if approximate_rotation == 90:
                ...
                target_row, target_column = directions[0][0]
        if cell['shape'] == 'cross':
            print(f'Approximation{approximate_rotation}')
            if approximate_rotation == 0:
                directions = [((current_row, current_column - 1),
                               distance(current_row, current_column - 1, trip_row, trip_column)),
                              ((current_row + 1, current_column),
...
def cozmo_program(robot: cozmo.robot.Robot):
    ...
    while True:
        ...
        current_row, current_column = row + start_row, column + start_column
        cell = get_cell(current_row, current_column)
        print(f'Current location: {cell}')
        try:
            if not action and cell['shape'] in ['tLeft', 'tRight', 'tTop', 'tBottom', 'cross']:
                get_new_target(current_row, current_column)
                approximations = [(0, 0 - rotation),
                                  (180, 180 - rotation),
                                  (90, 90 - rotation),
                                  (-90, -90 - rotation)]
                approximations.sort(key=lambda r : abs(r[1]))  # sort by rotation difference
                approximate_rotation = approximations[0][0]
                print(f'Target location: {get_cell(target_row, target_column)}')
                if approximate_rotation == 0:
                ...
                if action == 'turn-left':
                    ...
                    print(f'Target:({target_row}, {target_column})')
                    print(f'Current:({current_row}, {current_column})')
                    print(f'Previous:({previous_row}, {previous_column})')
